...
extern "C" {
    ...
    static void cs_unity_dispatch_on_main_thread(dispatch_block_t block)
    {
        if ( block )
        {
    ...
    static void cs_unity_send_message(const char* obj, const char* method, NSDictionary *json)
    {
        NSError *error;
